...
CREATE OR REPLACE PROCEDURE SP_OBTENER_FACTURAS(FACTURAS OUT SYS_REFCURSOR, RESULTADO OUT NUMBER, MENSAJE OUT VARCHAR2) AS 
BEGIN
OPEN FACTURAS FOR 
...
OUT_TOTAL OUT DECIMAL, OUT_MEDIO_DE_PAGO OUT VARCHAR2, OUT_FECHA_HORA_VENTA OUT TIMESTAMP, DETALLES OUT SYS_REFCURSOR, RESULTADO OUT NUMBER, MENSAJE OUT VARCHAR2) AS 
BEGIN
...
CREATE OR REPLACE PROCEDURE SP_INSERTAR_PERSONA (IN_CEDULA IN NUMBER, IN_NOMBRE IN VARCHAR2, IN_PRIMER_APELLIDO IN VARCHAR2, 
...
CREATE OR REPLACE PROCEDURE SP_OBTENER_PRODUCTOS(IN_ID_PRODUCTO IN NUMBER , OUT_ID_PRODUCTO OUT VARCHAR2, OUT_CODIGO OUT VARCHAR2,OUT_NOMBRE OUT VARCHAR2,
OUT_CATEGORIA OUT VARCHAR2, OUT_DETALLE OUT VARCHAR2, OUT_PRECIO OUT DECIMAL, OUT_TAMANO OUT VARCHAR2, OUT_CANTIDAD OUT NUMBER, RESULTADO OUT NUMBER, MENSAJE OUT VARCHAR2) AS 
BEGIN
...
OUT_DETALLE OUT VARCHAR2, OUT_PRECIO OUT DECIMAL, OUT_TAMANO OUT VARCHAR2, OUT_CANTIDAD OUT NUMBER, RESULTADO OUT NUMBER, MENSAJE OUT VARCHAR2) AS 
BEGIN
SELECT * INTO OUT_ID_PRODUCTO,
OUT_CODIGO,
OUT_NOMBRE,
...
CREATE OR REPLACE PROCEDURE SP_INSERTAR_PRODUCTO (IN_CODIGO IN VARCHAR2, IN_CATEGORIA_ID IN NUMBER, IN_MARCA_ID IN NUMBER,
IN_NOMBRE IN VARCHAR2, IN_DETALLE IN VARCHAR2, IN_PRECIO IN DECIMAL, IN_TAMANO IN NUMBER, IN_CANTIDAD IN NUMBER, RESULTADO OUT NUMBER, MENSAJE OUT VARCHAR2) AS
BEGIN
...
CREATE OR REPLACE PROCEDURE SP_MODIFICAR_PRODUCTO (IN_ID_PRODUCTO IN NUMBER, IN_CODIGO IN VARCHAR2, IN_CATEGORIA_ID IN NUMBER , IN_MARCA_ID IN NUMBER, 
...
EXCEPTION
WHEN OTHERS THEN
    MENSAJE := 'No se pudo modificar el producto ya que el nuevo nombre esta siendo utilizado por otro en el sitema';
    RESULTADO := 1;
END;
/
...
EXCEPTION
WHEN OTHERS THEN
    MENSAJE := 'No se pudo modificar la marca ya que el nuevo nombre esta siendo utilizado por otra en el sitema';
    RESULTADO := 1;
END;
/
...
CREATE OR REPLACE PROCEDURE SP_OBTENER_UNA_CATEGORIA (V_ID_CAT IN NUMBER, CATEGORIA OUT VARCHAR2, RESULTADO OUT NUMBER, MENSAJE OUT VARCHAR2) AS
BEGIN
...
CREATE OR REPLACE PROCEDURE SP_OBTENER_SUBCATEGORIAS_EN_CATEGORIA (V_ID_CAT IN NUMBER, SUB_CATS OUT SYS_REFCURSOR, RESULTADO OUT NUMBER, MENSAJE OUT VARCHAR2)
AS
BEGIN
...
EXCEPTION
WHEN OTHERS THEN
    MENSAJE := 'No se pudo modificar la categoria ya que el nuevo nombre esta siendo utilizado por otra en el sitema';
    RESULTADO := 1;
END;
/
...
CREATE OR REPLACE TYPE FACTURA_ARR AS VARRAY(200) OF OBJ_DETALLE_FACTURA;
/

CREATE OR REPLACE TYPE FACTURA_ARR_OBJ AS TABLE OF OBJ_DETALLE_FACTURA;
/
 
CREATE TABLE TAB_TEST_FACTURA (
ProductoID NUMBER,
Cantidad NUMBER(3),
Precio  NUMBER(8, 2),
IVA NUMBER(2,2)
...
CREATE OR REPLACE PROCEDURE TEST_FACTURA (IN_FACTURA_ARR_OBJ IN FACTURA_ARR_OBJ , RESULTADO OUT NUMBER)AS 
BEGIN
      FOR i IN 1..IN_FACTURA_ARR_OBJ.COUNT LOOP
        INSERT INTO TAB_TEST_FACTURA (ProductoID,Cantidad,Precio,IVA) VALUES (IN_FACTURA_ARR_OBJ(i).ProductoID, IN_FACTURA_ARR_OBJ(i).Cantidad, IN_FACTURA_ARR_OBJ(i).Precio, IN_FACTURA_ARR_OBJ(i).IVA);
      END LOOP;
RESULTADO :=0;
END;
/

CREATE OR REPLACE PROCEDURE TEST_DETALLE (IN_TEST_DETALLE IN OBJ_DETALLE_FACTURA, RESULTADO OUT NUMBER)AS 
BEGIN
RESULTADO :=0;
END;
/

CREATE OR REPLACE PROCEDURE TEST_OBJ_TABLE (IN_TEST_TABLE IN OBJ_DETALLES_FACTURA, RESULTADO OUT NUMBER)AS 
BEGIN
RESULTADO :=0;
END;
/
...
VAR  vtest2 varchar2(100);
exec SP_ELIMINAR_PRODUCTO (4, :vtest, :vtest2 );
print vtest;
*/
--SELECT * FROM TAB_TEST_FACTURA;

--DELETE FROM TAB_TEST_FACTURA;
